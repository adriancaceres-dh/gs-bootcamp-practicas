...
public class ClientService implements IClientService {
    ...
    public ClientDTO saveEntity(ClientDTO personDTO) {
        ...
        return mapper.map(clientRepository.save(entity), ClientDTO.class);
    }

    @Override
    public ClientDTO getEntityById(Integer integer) {
        var entity = clientRepository.findById(integer).orElseThrow(
                () -> {throw new RuntimeException("No se pudo encontrar ningun cliente con este ID " + integer);}
        );
    ...
    public MessageDTO deleteEntity(Integer integer) {
        ...
        if (clientRepository.existsById(integer)) {
            ...
            return MessageDTO.builder()
                    .message("No se pudo encontrar al cliente con ID " + integer)
                    .action("DELETATION")
                    .build();
        }
    }
